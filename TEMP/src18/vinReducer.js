...
export const checkVin = (vinNumber) => async (dispatch, getState) => {
    ...
    //тут надо переделать и эти две проверки удалть. просто обозначить начальные значения null в этих переменных и их диспатчить если не засетились
    if(responseNode1.data === undefined){
        dispatch(setDataArray(null));
    }

    if(linksArray !== undefined){
        dispatch(setImgLinks(null));
    }
